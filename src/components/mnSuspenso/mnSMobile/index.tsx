    ...
    return(
        <div className={`md:hidden flex justify-center items-center w-[60px] h-[60px] p-[10px] rounded-full transition-all duration-500 ease-in-out z-40 ${aberto ? "" : overFooter ? "bg-[rgba(128,128,128,0.05)] backdrop-blur-xs shadow-[inset_2px_2px_8px_rgba(255,255,255,0.08),2px_8px_10px_rgba(0,0,0,0.08)] hover:shadow-[inset_2px_2px_8px_rgba(255,255,255,0.08),2px_8px_12px_rgba(0,0,0,0.15)]" : scrolled ? "bg-[rgba(128,128,128,0.05)] backdrop-blur-xs shadow-[inset_2px_2px_8px_rgba(255,255,255,0.1),2px_8px_10px_rgba(0,0,0,0.08)]" : ""}`}>
          <div className={`absolute flex justify-center items-center w-0 h-0 bg-[rgba(245,236,219,0.95)] backdrop-blur-xs transition-all duration-600 ease-in-out rounded-[100%] ${aberto ? "right-[0px] top-[0px] w-[100vw] h-[100vh] p-[40px_40px_100px_40px] rounded-none" : ""}`}>
            <nav className={`w-full h-full flex-col justify-end items-start gap-[40px] list-none ${aberto ? 'flex' : 'hidden'}`}>
              <a href='#meu-objetivo' onClick={() => setAberto(false)} className='links-mn-mobile slideMenu1'>Meu objetivo</a>
              <a href='#demo' onClick={() => setAberto(false)} className='links-mn-mobile slideMenu2'>Demo.</a>
              <a href='#footer' onClick={() => setAberto(false)} className='links-mn-mobile slideMenu3'>Contato</a>
              <a href='#header' onClick={() => setAberto(false)} className="font-light hover:translate-y-[-4px] hover:transition-all rounded-[50px] text-shadow-[0px_0px_10px_rgba(0,0,0,0.3)] transition-all duration-500 slideMenu4">
              ⓒ Code by É. Adão
              </a>
            </nav>
          </div>
          <button 
            name='menu-suspenso-mobile' 
            onClick={() => setAberto(!aberto)} 
            className="relative w-[30px] h-[30px] transition-all duration-800 ease-in-out"
