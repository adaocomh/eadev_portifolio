...
export default function MenuSuspenso() {
  ...
  return (
    <div className="flex justify-between items-center w-[100vw] p-[0px_80px] transition-all duration-500">
      <div className={`hover:translate-y-[-4px] hover:transition-all hover:shadow-[0px_3px_8px_rgba(0,0,0,0.3)] p-[20px] rounded-[50px] transition-all duration-600 ${overFooter ? "bg-transparent backdrop-blur-md text-(--cor-primaria) border-[0.1px] borde-(--cor-primaria) shadow-[0px_8px_10px_rgba(0,0,0,0.10)]" : scrolled ? "bg-transparent backdrop-blur-md shadow-[0px_8px_10px_rgba(0,0,0,0.05)]" : ""}`}>
        <p className="cursor-pointer font-light">
          ⓒ Code by É. Adão
        </p>
      </div>
      <nav>
        <ul className={`hidden rounded-[50px] md:flex md:flex-row md:gap-[20px] md:p-[20px] transition-all duration-600 ${overFooter ? "bg-transparent backdrop-blur-md text-(--cor-primaria) border-[0.1px] borde-(--cor-primaria) shadow-[0px_8px_10px_rgba(0,0,0,0.10)]" : scrolled ? "bg-transparent backdrop-blur-md shadow-[0px_8px_10px_rgba(0,0,0,0.05)]" : ""}`}>
